# url编码，是浏览器未了防止非法格式的url而导致bug，浏览器会自动将非法的字符进行url编码
#   我们在修改备注时，是将修改内容拼接到url上的，一些中文、符号都会被浏览器自动进行url编码
#   因此保存到数据库中的数据，都是经过url编码的
#   想要正确输出内容，需要将编码的内容进行解码

import urllib.parse
# url编码
#   urllib.parse.quote('中国')
#   返回值：'%E4%B8%AD%E5%9B%BD'
# url解码
