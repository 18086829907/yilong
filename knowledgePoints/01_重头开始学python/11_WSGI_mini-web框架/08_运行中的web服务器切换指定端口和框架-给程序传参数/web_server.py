...
def main():
    if len(sys.argv) == 3:
        ...
        print('python3 web_server 8080 mini_frame:application')
        return

    res = re.match('([^:]+):(.*)',frame_app_name)
    if res:
    ...
    sys.path.append('./dynamic')  # 将当前工作目录中的dynamic文件包加载到模块搜索路径，即导入模块时会到dynamic包中去寻找模块
