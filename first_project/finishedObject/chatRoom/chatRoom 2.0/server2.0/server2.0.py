...
def sendMail(mail):
    mailStr = mail.decode('utf-8')  # dataStr = friend,userName说: you are very good!
...
def signalRun(clientSocket, clientAddress):
    userName = clientSocket.recv(1024).decode('utf-8') #等待接收UserName
    users[userName] = clientSocket #保存客户端发送过来的用户名和该客户端的客户端对象
    for client in list(users.keys()):
            users[client].send(str(list(users.keys())).encode('utf-8'))
    while True:
        mail = clientSocket.recv(2014) #阻塞程序等待接收消息
        t = threading.Thread(target=sendMail, args=(mail,))
        t.start()
